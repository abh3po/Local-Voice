...
def get_input_device_index(preferred_name="Shure MVX2U"):
    ...
    for i in range(pa.get_device_count()):
        ...
        if preferred_name.lower() in info['name'].lower() and info['maxInputChannels'] > 0:
            ...
            print(
                f"[Debug] Device sample rate: {info['defaultSampleRate']} Hz")
            index = i
            break
    pa.terminate()
    if index is None:
...
def list_input_devices():
    ...
    for i in range(pa.get_device_count()):
        ...
        if info['maxInputChannels'] > 0:
            print(
                f"  {i}: {info['name']} ({int(info['defaultSampleRate'])} Hz, {info['maxInputChannels']}ch)")
    pa.terminate()
...
TOOLS = {
    ...
    "set_timer": lambda duration, message="Time is up!": set_timer_tool(duration, message, play_response)
}
...
print(
    f"[Debug] Config loaded: model={MODEL_NAME}, voice={config['voice']}, vol={VOLUME}, mic={MIC_NAME}")
